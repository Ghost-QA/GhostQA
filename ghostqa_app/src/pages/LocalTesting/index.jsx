...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (!executingSuite && selectedSuite) {
      dispatch(getTestCaseRundetailsByTestName(selectedSuite, setInProgress));
    }
  }, [executingSuite, selectedSuite]);

  useEffect(() => {
    dispatch(fetchWorkSpaces());
  }, [dispatch]);

  // Filter the test suite data based on selected tab and search term
  const filteredTestSuiteData = testSuits?.filter((suite) =>
    selectedTab === "inbuilt"
      ? suite.TestSuiteFlag === "InBuilt" &&
        ...
        <Grid container spacing={2}>
                      ...
                      filteredTestSuiteData.map((suite) => (
                        <Paper
                          ...
                          className={`${classes.paper} ${
                            selectedSuite?.suiteName === suite.TestSuiteName
                              ? classes.paperActive
                              : ""
                          }`}
          ...
          <Grid item xs={12} sm={8}>
            {inprogress ? (
              ...
              <>
                {selectedSuite && selectedTab === "inbuilt" && (
